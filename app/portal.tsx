...
const abi = [
  ...
  { type:"function", name:"batchBuy", stateMutability:"payable", inputs:[{name:"nftAddresses",type:"address[]"},{name:"tokenIds",type:"uint256[]"}], outputs:[] },
  { type:"function", name:"makeOffer", stateMutability:"payable", inputs:[{name:"nftAddress",type:"address"},{name:"tokenId",type:"uint256"},{name:"expiresAt",type:"uint64"}], outputs:[] },
  { type:"function", name:"marketplaceVersion", stateMutability:"pure", inputs:[], outputs:[{name:"",type:"uint256"}] },
...
export function Portal({ view }: { view:View }) {
  ...
  const advancedMarketplace=marketplaceVersion===2n;
  ...
  async function batchBuy(items:Listing[]){if(!address||!data.marketplaceAddress||!advancedMarketplace||!items.length)return;try{await ensureChain();const total=items.reduce((sum,item)=>sum+BigInt(item.price),0n);setStatus(`Confirm one batch purchase for ${items.length} NFTs.`);const hash=await writeContractAsync({address:data.marketplaceAddress,abi,functionName:"batchBuy",args:[items.map(item=>item.nftAddress),items.map(item=>BigInt(item.tokenId))],value:total,chainId:selectedChainId});setStatus(`Batch purchase submitted · ${short(hash)}`);window.setTimeout(refresh,5_000);}catch(e){setStatus(e instanceof Error?e.message.split("\n")[0]:"Batch purchase failed");}}
  async function makeOffer(item:Listing,amount:string){if(!address||!data.marketplaceAddress||!advancedMarketplace)return;try{await ensureChain();const expiresAt=BigInt(Math.floor(Date.now()/1000)+7*24*60*60);setStatus("Confirm your seven-day offer in your wallet.");const hash=await writeContractAsync({address:data.marketplaceAddress,abi,functionName:"makeOffer",args:[item.nftAddress,BigInt(item.tokenId),expiresAt],value:parseEther(amount),chainId:selectedChainId});setStatus(`Offer submitted · ${short(hash)}`);}catch(e){setStatus(e instanceof Error?e.message.split("\n")[0]:"Offer failed");}}
  ...
  return <main className={`portal-page portal-${view}`}>
    ...
    <section className="portal-content">
      {view==="market"&&<MarketView data={data} loading={loading} account={address} advancedMarketplace={advancedMarketplace} onBuy={buy} onBatchBuy={batchBuy} onOffer={makeOffer} onCancel={cancel}/>}
      {view==="sell" && (
...
function MarketView({data,loading,account,advancedMarketplace,onBuy,onBatchBuy,onOffer,onCancel}:{data:IndexerData;loading:boolean;account?:`0x${string}`;advancedMarketplace:boolean;onBuy:(x:Listing)=>void;onBatchBuy:(items:Listing[])=>void;onOffer:(item:Listing,amount:string)=>void;onCancel:(x:Listing)=>void}){
  ...
  const cart=data.listings.filter(item=>cartIds.includes(item.id));
  ...
  return <>
    ...
    <div className={`market-browser ${filtersOpen?"":"filters-closed"}`}>
      ...
      <section className="market-results">
        {shown.length?<div className="market-listings-grid">{shown.map(item=><MarketListingCard key={item.id} item={item} currency={data.currency} chain={data.chain} inCart={cartIds.includes(item.id)} onOpen={()=>setActive(item)} onToggleCart={()=>toggleCart(item)}/>)}</div>:<div className="market-no-results"><Search size={24}/><h2>No NFTs found</h2><p>Try another token ID, contract address, or collection.</p><button onClick={()=>{setSearch("");setCollection("all")}}>Clear filters</button></div>}
      </section>
    </div>
    {active&&<MarketNftDetail listing={active} details={details} account={account} advancedMarketplace={advancedMarketplace} onClose={()=>setActive(null)} onBuy={()=>onBuy(active)} onOffer={amount=>onOffer(active,amount)} onDelist={()=>{onCancel(active);setActive(null);}}/>}
...
function MarketListingCard({item,currency,chain,inCart,onOpen,onToggleCart}:{item:Listing;currency:string;chain:string;inCart:boolean;onOpen:()=>void;onToggleCart:()=>void}){
  ...
  return <article className="market-listing">
    ...
    <div>
      ...
      <div className="market-card-actions"><button onClick={onOpen}>Buy now <ArrowUpRight size={13}/></button><button className={inCart?"active":""} onClick={onToggleCart} aria-label={inCart?"Remove from cart":"Add to cart"}><ShoppingCart size={14}/>{inCart?"Added":"Add"}</button></div>
    </div>
  </article>
}
...
function SellView({configured,connected,walletNfts,onList,chainId}:{configured:boolean;connected:boolean;walletNfts:{nfts:WalletNft[];loading:boolean;error:string};onList:(n:string,t:string,p:string)=>void;chainId:MarketplaceChainId}){
  ...
  const listableNfts=walletNfts.nfts.filter(item=>(item.tokenType??"ERC-721")==="ERC-721");
...
function AccountView({connected,configured,listings,activity,account,proceeds,onCancel,onWithdraw,currency,walletNfts,chainId}:{connected:boolean;configured:boolean;listings:Listing[];activity:Activity[];account?:`0x${string}`;proceeds:bigint;onCancel:(x:Listing)=>void;onWithdraw:()=>void;currency:string;walletNfts:{nfts:WalletNft[];loading:boolean;error:string};chainId:MarketplaceChainId}){
  ...
  const history=activity.filter(item=>account&&item.eventType==="sold"&&(item.seller?.toLowerCase()===account.toLowerCase()||item.buyer?.toLowerCase()===account.toLowerCase()));
