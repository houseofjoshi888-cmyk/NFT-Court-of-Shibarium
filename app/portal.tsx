...
export function Portal({ view }: { view:View }) {
  ...
  const labels:Record<View,[string,string,string]>= { market:["01 / MARKET","Works before the court",`Active listings verified from confirmed ${selectedChain.name} events.`], sell:["02 / PRESENT","Present a work",`Approve and list an ERC-721 on ${selectedChain.name}. The NFT remains in your wallet until sold.`], activity:["03 / ACTIVITY","The onchain record",`Listings, sales, cancellations, and withdrawals on ${selectedChain.name}.`], account:["04 / ACCOUNT","Your court account",`Manage ${selectedChain.name} listings and withdraw settled ${selectedChain.currency} proceeds.`], protocol:["05 / PROTOCOL","Rules of the court","Transparent settlement, fees, royalties, and ownership mechanics."] };
...
function AccountView({connected,configured,listings,proceeds,onCancel,onWithdraw,currency}:{connected:boolean;configured:boolean;listings:Listing[];proceeds:bigint;onCancel:(x:Listing)=>void;onWithdraw:()=>void;currency:string}){if(!connected)return <ConnectButton.Custom>{({openConnectModal})=><Empty eyebrow="WALLET REQUIRED" title="Connect to enter your account." detail="Your active listings and withdrawable proceeds will appear here." action={<button onClick={openConnectModal}>Connect wallet <Wallet size={15}/></button>}/>}</ConnectButton.Custom>;return <><div className="account-balance"><span>WITHDRAWABLE PROCEEDS</span><strong>{formatEther(proceeds)} {currency}</strong><button disabled={!configured||proceeds===0n} onClick={onWithdraw}>Withdraw proceeds <ArrowUpRight/></button></div><div className="account-section"><span className="section-no">ACTIVE LISTINGS</span>{listings.length?<div className="portal-grid account-grid">{listings.map(x=><ListingTile key={x.id} item={x} action={<button onClick={()=>onCancel(x)}>Cancel listing</button>}/>)}</div>:<p className="account-empty">You have no active listings on this network.</p>}</div></>}
