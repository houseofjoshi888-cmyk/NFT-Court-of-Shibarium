...
function useNftMetadata(listing: Listing | null) {
  ...
  const [loading,setLoading]=useState(false);
  const [error,setError]=useState("");
  useEffect(()=>{let active=true;void(async()=>{if(!listing){setNft(null);setError("");return;}setLoading(true);setError("");try{const response=await fetch(`/api/nft?contract=${listing.nftAddress}&tokenId=${listing.tokenId}`);const body=await response.json() as WalletNft&{error?:string};if(!response.ok)throw new Error(body.error??"Could not load NFT metadata.");if(active)setNft(body);}catch(e){if(active)setError(e instanceof Error?e.message:"Could not load NFT metadata.");}finally{if(active)setLoading(false);}})();return()=>{active=false;};},[listing]);
...
function PortalHeader(){return <header className="portal-header"><Link href="/" className="wordmark"><Image src="/house-of-joshi.png" alt="House of Joshi" width={42} height={42} className="header-logo"/><span>HOUSE OF JOSHI</span></Link><nav><Link href="/market">Market</Link><Link href="/sell">Sell</Link><Link href="/activity">Activity</Link><Link href="/account">Account</Link><Link href="/protocol">Protocol</Link></nav><CourtConnect/></header>}
...
function MarketView({data,loading,account,onBuy,onCancel}:{data:IndexerData;loading:boolean;account?:`0x${string}`;onBuy:(x:Listing)=>void;onCancel:(x:Listing)=>void}){const[collection,setCollection]=useState("all");const[active,setActive]=useState<Listing|null>(null);const details=useNftMetadata(active);if(!data.listings.length)return <Empty eyebrow={loading?"SYNCING SHIBARIUM":data.configured?"NO ACTIVE LISTINGS":"SETUP REQUIRED"} title={loading?"Reading the onchain record.":data.configured?"No works are listed.":"Connect the deployed contract."} detail={data.syncError||"Only verified active listings appear here."}/>;const collections=[...new Set(data.listings.map(item=>item.nftAddress))];const shown=collection==="all"?data.listings:data.listings.filter(item=>item.nftAddress===collection);return <><section className="collection-picker"><div><span>COLLECTIONS</span><small>{collections.length} collection{collections.length===1?"":"s"} · {data.listings.length} active works</small></div><div className="collection-chips"><button className={collection==="all"?"active":""} onClick={()=>setCollection("all")}>All works</button>{collections.map(address=><button className={collection===address?"active":""} key={address} onClick={()=>setCollection(address)}>{short(address)}</button>)}</div></section><div className="market-listings-grid">{shown.map(item=><article key={item.id} className="market-listing"><button className="market-listing-art" onClick={()=>setActive(item)}><span>ERC-721</span><strong>#{item.tokenId}</strong><i>{short(item.nftAddress)}</i></button><div><small>{short(item.nftAddress)}</small><h2>Token #{item.tokenId}</h2><p>{formatEther(BigInt(item.price))} BONE</p><button onClick={()=>setActive(item)}>View work <ArrowUpRight size={13}/></button></div></article>)}</div>{active&&<MarketNftDetail listing={active} details={details} account={account} onClose={()=>setActive(null)} onBuy={()=>onBuy(active)} onDelist={()=>{onCancel(active);setActive(null);}}/>}</>}

function MarketNftDetail({listing,details,account,onClose,onBuy,onDelist}:{listing:Listing;details:{nft:WalletNft|null;loading:boolean;error:string};account?:`0x${string}`;onClose:()=>void;onBuy:()=>void;onDelist:()=>void}){const nft=details.nft;const isSeller=!!account&&account.toLowerCase()===listing.seller.toLowerCase();return <div className="nft-detail-backdrop" role="dialog" aria-modal="true" aria-label="Listed NFT details" onMouseDown={e=>{if(e.target===e.currentTarget)onClose();}}><article className="nft-detail"><button className="nft-detail-close" onClick={onClose} aria-label="Close NFT details"><X size={18}/></button>{nft?.imageUrl?<div className="nft-detail-image" style={{backgroundImage:`url(${nft.imageUrl})`}}/>:<div className="nft-detail-image empty"/>}<div className="nft-detail-copy"><small>{nft?.collection??short(listing.nftAddress)} · ERC-721</small><h2>{nft?.name??`Token #${listing.tokenId}`}</h2><p className="nft-token-id">#{listing.tokenId} · {short(listing.nftAddress)}</p>{details.loading&&<p className="nft-description">Loading verified metadata…</p>}{details.error&&<p className="nft-description">{details.error}</p>}{nft?.description&&<p className="nft-description">{nft.description}</p>}{nft&&nft.traits.length>0&&<section className="nft-traits"><span>TRAITS</span><div>{nft.traits.map(trait=><article key={`${trait.type}:${trait.value}`}><small>{trait.type}</small><strong>{trait.value}</strong></article>)}</div></section>}<div className="listing-price-panel"><span>LISTING PRICE</span><strong>{formatEther(BigInt(listing.price))} BONE</strong><small>Seller · {short(listing.seller)}</small></div><div className="nft-detail-actions">{isSeller?<button className="delist-button" onClick={onDelist}>Delist NFT <X size={15}/></button>:account?<button onClick={onBuy}>Buy now <ArrowUpRight size={15}/></button>:<ConnectButton.Custom>{({openConnectModal})=><button onClick={openConnectModal}>Connect to buy <Wallet size={15}/></button>}</ConnectButton.Custom>}{nft?.externalUrl&&<a href={nft.externalUrl} target="_blank" rel="noreferrer">Collection link <ExternalLink size={14}/></a>}<a href={`https://shibariumscan.io/token/${listing.nftAddress}/instance/${listing.tokenId}`} target="_blank" rel="noreferrer">View on ShibariumScan <ExternalLink size={14}/></a></div></div></article></div>}
...
function PortalFooter(){return <footer className="portal-footer"><div className="footer-main"><Link href="/" className="wordmark"><Image src="/house-of-joshi.png" alt="House of Joshi" width={36} height={36} className="footer-logo"/><span>HOUSE OF JOSHI</span></Link><p>Curated, non-custodial NFT trading on Shibarium.</p></div><div><span>MARKETPLACE</span><Link href="/market">Market</Link><Link href="/sell">Sell</Link><Link href="/activity">Activity</Link><Link href="/faq">FAQ</Link><Link href="/about">About</Link></div><div><span>HOUSE ECOSYSTEM</span><a href="https://kingdomwithin.thehouseofjoshi.com/" target="_blank" rel="noreferrer">Kingdom Within</a><a href="https://swap.thehouseofjoshi.com/" target="_blank" rel="noreferrer">HOJ Swap</a><a href="https://www.nftlaunchpad.thehouseofjoshi.com/" target="_blank" rel="noreferrer">NFT Launchpad</a><a href="https://dreamweaver.thehouseofjoshi.com/" target="_blank" rel="noreferrer">Dreamweaver</a></div><div><span>CONNECT</span><Link href="/contact">Contact</Link><a href="https://x.com/thehouseofjoshi" target="_blank" rel="noreferrer">X · @thehouseofjoshi</a><a href="https://discord.com/invite/uH9zVeAwDu" target="_blank" rel="noreferrer">Discord · Join the community</a><a href="https://www.instagram.com/thehouseofjoshi" target="_blank" rel="noreferrer">Instagram · @thehouseofjoshi</a></div><div className="footer-legal"><span>© 2026 The House of Joshi. All rights reserved.</span><Link href="/terms">Terms &amp; Conditions</Link><Link href="/privacy">Privacy Policy</Link></div></footer>}
