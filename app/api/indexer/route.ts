  ...
  for (const log of logs) {
    try {
      ...
      if (decoded.eventName === "ItemListed") {
        ...
        const { buyer, nftAddress, tokenId, price, marketplaceFee, royaltyRecipient, royaltyAmount } = decoded.args;
        const soldListingId=listingId(chainId,nftAddress,tokenId);
        statements.push(db.prepare("INSERT OR IGNORE INTO multichain_marketplace_activity (id, chain_id, event_type, nft_address, token_id, seller, buyer, price, marketplace_fee, royalty_recipient, royalty_amount, transaction_hash, block_number, log_index) VALUES (?, ?, 'sold', ?, ?, (SELECT seller FROM multichain_listings WHERE id=?), ?, ?, ?, ?, ?, ?, ?, ?)").bind(eventId, chainId, nftAddress, String(tokenId), soldListingId, buyer, String(price), String(marketplaceFee), royaltyRecipient, String(royaltyAmount), log.transactionHash, blockNumber, logIndex));
        statements.push(db.prepare("UPDATE multichain_listings SET active=0, transaction_hash=?, updated_block=? WHERE id=?").bind(log.transactionHash, blockNumber,soldListingId));
      } else {
        const { seller, amount } = decoded.args;
