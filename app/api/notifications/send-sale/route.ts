...
export async function POST(request: Request) {
  try {
    const { email, nftName, price, currency, buyer, transactionUrl, collectionName } = await request.json() as {
      email?: string;
      nftName?: string;
      price?: string;
      currency?: string;
    ...
    if (!resendApiKey) {
      ...
      console.log("Sale notification details:", { nftName, price, currency, buyer, transactionUrl, collectionName });
        ...
        html: `
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; background-color: #0A0A0A; color: #FFFFFF;">
            <div style="text-align: center; padding: 30px 0; border-bottom: 2px solid #D4AF37;">
              <h1 style="color: #D4AF37; margin: 0; font-size: 32px;">🎉 Your NFT was sold!</h1>
            ...
            <div style="padding: 30px 0;">
              <div style="background: #1E1E1E; padding: 20px; border-radius: 12px; margin-bottom: 20px;">
                <h2 style="color: #D4AF37; margin-top: 0;">Sale Details</h2>
                ...
                <div style="margin: 15px 0;">
                  <span style="color: #888; display: block; margin-bottom: 5px;">Sale Price</span>
                  <strong style="font-size: 24px; color: #D4AF37;">${price} ${currency}</strong>
                </div>
                
                ${buyer ? `
                <div style="margin: 15px 0;">
                  <span style="color: #888; display: block; margin-bottom: 5px;">Buyer</span>
                  <code style="background: #121212; padding: 8px 12px; border-radius: 4px; color: #D4AF37;">${buyer.slice(0, 6)}…${buyer.slice(-4)}</code>
                </div>
                ` : ''}
              </div>
              ...
              <div style="border-top: 1px solid #333; padding-top: 20px; text-align: center;">
                <p style="color: #666; margin: 0; font-size: 14px;">
                  You received this email because you have email notifications enabled for NFT sales on House of Joshi Marketplace.
                </p>
    ...
    return Response.json({ error: "Failed to send sale notification." }, { status: 500 });
  }
}